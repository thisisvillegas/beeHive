...
void taskNetwork(void* parameter) {
    ...
    if (wifiManager.begin()) {
        ...
        // Start web server (also loads config)
        setWebServerWifiManager(&wifiManager);
        setupWebServer();

        // Start MQTT with config from web server
        setupMqtt(getConfigMqttBroker(), getConfigMqttPort(), deviceId);

        // Set companion bee if configured
        const char* companion = getConfigCompanionId();
        if (strlen(companion) > 0) {
            setCompanionBee(companion);
        }
    } else {
    ...
    while (true) {
        ...
        if (now - lastHealth >= 5000) {
            lastHealth = now;
            publishHealth(WiFi.RSSI(), ESP.getFreeHeap());
        }
